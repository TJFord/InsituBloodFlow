...
public:
    DynamicBoundaryFunctional(plint zc_, plint yc_, plint radius_, plint dr_, plint it_) : zc(zc_), yc(yc_), radius(radius_), dr(dr_), it(it_) { }
            ...
            for (plint iY = domain.y0; iY <= domain.y1; ++iY) {
                for (plint iZ = domain.z0; iZ <= domain.z1; ++iZ) {
                    T dist = (iY + relativeOffset.y - yc)*(iY + relativeOffset.y - yc) + 
                             (iZ + relativeOffset.z - zc)*(iZ + relativeOffset.z - zc); 

                    if (dist < (radius - dr*cos(it))*(radius-dr*cos(it))) {
...
void createDynamicBoundaryFromDataProcessor(
    MultiBlockLattice3D<T, DESCRIPTOR> &lattice, plint zc, plint yc, plint radius, plint dr, plint it)
{
    applyProcessingFunctional(
        new DynamicBoundaryFunctional<T, DESCRIPTOR>(zc,yc, radius,dr, it), lattice.getBoundingBox(),
        lattice);
}
...
/*
    ...
    plint zc,yc,radius,dr;
    zc = 20; yc=20; radius=10; dr=6;
 ...
 //           writeVTK(lattice, parameters, 4e3);
    ...
    //for (plint iT=0; iT<2; ++iT) {
        ...
        createDynamicBoundaryFromDataProcessor(lattice, zc, yc, radius, dr, 0.04*iT);
