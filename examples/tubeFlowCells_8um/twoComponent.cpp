...
T poiseuilleVelocityCircular(plint iX, plint iY, IncomprFlowParam<T> const& parameters, plint r){
    const T a = parameters.getNx()-1;
    const T b = parameters.getNy()-1;
...
class WallDomain3D: public DomainFunctional3D{
  public:
    WallDomain3D(vector< vector<plint> > coor_, plint r_):
    coor(coor_),r(r_){}
  virtual bool operator ()(plint iX, plint iY, plint iZ) const {
    plint result = 0;
    for (plint i=0;i<coor.size();i++){
      T rSqr = util::sqr(iX-coor[i][0]) + util::sqr(iY-coor[i][1]);
      if (rSqr >= r*r)
        result = 1;
    }
    return result; 
                            ...
                            OnLatticeBoundaryCondition3D<T,DESCRIPTOR>& boundaryCondition )
{
    ...
    Box3D inlet    = Box3D(1,    nx-1, 1,    ny-2, 0,    0);
    Box3D outlet = Box3D(1,    nx-1, 1,    ny-2, nz-1, nz-1);
    ...
    */
    ...
    setBoundaryDensity(lattice, inlet, 1.001);
    setBoundaryDensity(lattice, outlet, 1.);
    setBoundaryDensity(lattice, left, 1.0003);
    */

    defineDynamics(lattice, lattice.getBoundingBox(), new WallDomain3D<T>(coor,r),new BounceBack<T,DESCRIPTOR>);
...
/*
    ...
    const T maxT     =1000001;//6.6e4; //(T)0.01;
    plint iSave = 2000;//2000;//10;
    plint iCheck = 20*iSave;
    writeLogFile(parameters, "3D square Poiseuille");
    //cell 
    LammpsWrapper wrapper(argv,global::mpi().getGlobalCommunicator());
    char * inlmp = argv[1];
    plint iniT = atoi(argv[2]);//read timestep
...
/*
    ...
    for (plint iT=iniT; iT<maxT; ++iT) {
    //for (plint iT=0; iT<2; ++iT) {
        if (iT%iSave ==0 && iT >0){
            pcout<<"Saving VTK file..."<<endl;
            writeVTK(lattice, parameters, iT);
        }
        if (iT%iCheck ==0 && iT >iniT){
        ...
        lattice.collideAndStream();
        computeVelocity(lattice,vel,lattice.getBoundingBox());
        ////// Interpolate and update solid position
        //interpolateVelocity3D(lattice,wrapper);
        interpolateVelocity3D(vel,wrapper);//velocity version
        //interpolateVelocity3D(vel,wrapper2);//velocity version
