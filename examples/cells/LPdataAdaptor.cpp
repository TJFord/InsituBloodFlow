...
static
svtkIntArray *newGhostCellsArray(plb::Box3D domain, int ng, int gnx, int gny, int gnz)
{
    ...
    svtkIntArray *g = svtkIntArray::New();
...
namespace senseiLP
{
  ...
  int LPDataAdaptor::GetMeshMetadata(unsigned int id, sensei::MeshMetadataPtr &metadata) 
  {
    ...
    if (id == 0) // id == 0 is cells
    {
      ...
      metadata->MeshType = SVTK_MULTIBLOCK_DATA_SET; //VTK_POLY_DATA;
  ...
  int LPDataAdaptor::AddGhostCellsArray(svtkDataObject* mesh, const std::string &meshName)
  {
    ...
    if(meshName == "fluid")
    {
      ...
      svtkIntArray *ga = newGhostCellsArray(this->Internals->domainBox, this->Internals->envelopeWidth , this->Internals->pb_nx, this->Internals->pb_ny, this->Internals->pb_nz);//XXX Fix this to envelopeWidth
      dsa->AddArray(ga);
      ga->Delete(); 
    }
